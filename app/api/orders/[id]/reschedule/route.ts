...
function isBeforeTomorrow(dateText: string) {
  const tomorrow = new Date();
  tomorrow.setDate(tomorrow.getDate() + 1);
  return dateText < kstDateOnly(tomorrow);
}
...
async function assertSlotAvailable(params: {
  ...
  if (isBeforeTomorrow(reservedDate)) {
    return fail("INVALID_DATE", "오늘과 내일은 예약 변경이 어렵습니다. 다른 날짜를 선택해주세요.", 400);
  }
